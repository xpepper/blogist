...
var crawlPage = function(idx, arr) {
  ...
  var uri = "http://xpepper.github.io/#/gist/" +  gist.id + "/" + gist.description;
	console.log(uri);
		https.get({host:"gist.github.com",path:"/xpepper/"+gist.id+".json"}, function(res) {
		...
		res.on('end',function(){
			...
			var url= "http://xpepper.github.io/gist/" + gist.id + "/" + postfix; // link to the item
...
https.get({host:"gist.github.com",path:"/xpepper.atom","headers": {'User-Agent':"Mozilla/5.0"}}, function(res) {
	res.setEncoding('utf8');
	var data = "";
  res.on('data', function (chunk) {
		data +=chunk;
...
https.get({host:"api.github.com",path:"/users/xpepper/gists","headers": {'User-Agent':"Mozilla/5.0","Content-Type":	"application/json"}}, function(res) {
	res.setEncoding('utf8');
	var data = "";
  res.on('data', function (chunk) {
		data +=chunk;
